...
def _apply() -> None:
    ...
    # import roc_curve`, and the object-detection loss modules do `from
    # scipy.optimize import linear_sum_assignment`. Both fire the moment a model
    # class loads, dragging sklearn + scipy (~0.4s) into every import. alloy runs
    # only causal-LM / embedding / vision / audio paths, so force both probes False.
